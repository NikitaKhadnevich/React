...
const Todoindex = (props) => {
    ...
    const addToDo = (e) => { 
      e.preventDefault()
      const { onClickADD, texts } = props // Создаем пропс для прокидки - забираем его из стэйта формы
    ...
    const editTodo = (e) => {
      ...
      const swapTodo = todo[dataset] // определеем данные элемента массива
      const itemTodo = todo.indexOf(swapTodo); // Проверяем на совпадение
      if (itemTodo !== -1) { // Если данные не изменены - то ничего не вернется
        todo[itemTodo] = texts;
      }
      onClickEDITTODO(todo) // Передаем данные в стэйт (массив тудушек)
      changeTodo(e) // И меняем по сути статус и скрываем блок
    }
